...
def employee_view():
    ...
    # Initialize session state for employee_messages
    ...
    for message in st.session_state.employee_messages:
        role_color = "background-color: #2e2c2c;"  # Gray background for all employee_messages
    ...
    if input := st.chat_input("Type your message here..."):
        st.session_state.employee_messages.append({"role": "user", "content": input})
        with chat_container:
